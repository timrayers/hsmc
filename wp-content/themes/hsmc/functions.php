...
/** HSMC Functions ************************************************************/
add_action( 'wp_enqueue_scripts', 'hsmc_enqueue_styles' );
function hsmc_enqueue_styles() {
  wp_enqueue_style( 'hsmc-sitewide', get_stylesheet_directory_uri() . '/css/style.css' );
}

function hsmc_widgets_init() {
	register_sidebar( array(
		'name'          => 'Home centre banner',
		'id'            => 'home_centre_banner',
		'before_widget' => '<div>',
		'after_widget'  => '</div>',
		'before_title'  => '<h2>',
		'after_title'   => '</h2>',
...
function register_footer_menu() {
  register_nav_menu('footer-menu',__( 'Footer Menu' ));
	register_nav_menu('alternative-header-menu',__( 'Alternative Header Menu' ));
}
add_action( 'init', 'register_footer_menu' );


function hsmc_show_notices_link($atts) {
	//[show_notices_link]
	if(file_exists(get_stylesheet_directory() . '/downloads/notices.pdf')) {
		if(array_key_exists('display',$atts)) {
			if($atts['display'] == 'inline') {
					return "<a class='btn notices_download' href='" . get_stylesheet_directory_uri() . "/downloads/notices.pdf?download=notices' target='_blank' class='open-notices'>View Notices</a>";
			} else {
					return null;
			}
		}	else {
...
add_shortcode( 'show_rota_link', 'hsmc_show_rota_link' );

/*
 * Hide the word 'Protected' or 'Private' from title
 */
...
function add_alt_menu(){
	wp_nav_menu( array(
		...
		'container'      => 'menu-alternative-wrapper',
